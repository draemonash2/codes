...
function _renamedirfiles() { # {{{
    # Rename all files and directories under the current directory.
    if [ $# -ne 3 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : _renamedirfiles <mv_cmd> <source_keyword> <destination_keyword>"
        return 1
    fi
    mv_cmd=$1
    srckeyword=$2
    ...
    do
        ...
        do
            ...
            #echo "${mv_cmd} -f ${srcdir} ${dstdir} 2> /dev/null"
            ${mv_cmd} -f ${srcdir} ${dstdir} 2> /dev/null
        done
        #echo ""
    done
    ...
    do
        ...
        do
            ...
            #echo "${mv_cmd} -f ${srcfile} ${dstfile} 2> /dev/null"
            ${mv_cmd} -f ${srcfile} ${dstfile} 2> /dev/null
        done
    done
...
function renamedirfiles() { # {{{
    ...
    srckeyword=$1
    dstkeyword=$2
    _renamedirfiles "mv" ${srckeyword} ${dstkeyword}
} # }}}
function renamedirfilesgit() { # {{{
    # Rename all git files and directories under the current directory.
    if [ $# -ne 2 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : renamedirfilesgit <source_keyword> <destination_keyword>"
        return 1
    fi
    srckeyword=$1
    dstkeyword=$2
    _renamedirfiles "git mv" ${srckeyword} ${dstkeyword}
} # }}}
function renamewords() { # {{{
    if [ $# -ne 2 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : renamewords <source> <destination>"
        return 1
    fi
...
function latestdirpath() { # {{{
    ...
    if [ $# -ge 2 ]; then
        ...
        if [ $# -ge 1 ]; then
            ...
            echo "[error] wrong number of arguments."
            echo "  usage : latestdirpath <base_dir_path> [<dir_name_pattern>]"
            return 1
        fi
    fi
    (
        ...
        if [ "${include_symlink}" -eq 1 ]; then
            dir_name=$(find . -name "${dir_name_pattern}" \( -type d -o -type l -xtype d \) | grep ./ | awk -F/ '{ print $2 }' | sort -n | tail -1)
        else
            dir_name=$(find . -name "${dir_name_pattern}" \( -type d \) | grep ./ | awk -F/ '{ print $2 }' | sort -n | tail -1)
        fi
        if [ "${dir_name}" != "" ]; then
...
function latestfilepath() { # {{{
    ...
    if [ $# -ge 2 ]; then
        ...
        if [ $# -ge 1 ]; then
            ...
            echo "[error] wrong number of arguments."
            echo "  usage : latestfilepath <base_dir_path> [<file_name_pattern>]"
            return 1
        fi
    fi
    (
        \cd ${base_dir_path}
        if [ "${include_symlink}" -eq 1 ]; then
            file_name=$(find . -name "${file_name_pattern}" \( -type f -o -type l -xtype f \) | grep ./ | awk -F/ '{ print $2 }' | sort -n | tail -1)
        else
            file_name=$(find . -name "${file_name_pattern}" \( -type f \) | grep ./ | awk -F/ '{ print $2 }' | sort -n | tail -1)
        fi
        if [ "${file_name}" != "" ]; then
...
function topicecho_robotdescsem() { # {{{
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : topicecho_robotdescsem <filepath>"
        return 1
    fi
    outfile=${1}
    ros2 topic echo /robot_description_semantic -f --once > ${outfile}; alignxml ${outfile}
} # }}}
...
function outputtftree() { # {{{
    ...
    if [ "${file_base_name}" == "" ]; then
        ...
        ros2 run tf2_tools view_frames -o ${file_base_name}
    fi
} # }}}
...
function latestros2alllog() { # {{{
    BASEDIRPATH=${HOME}/.ros/log
    logfiledir=$(latestdirpath ${BASEDIRPATH})
    logfile=${logfiledir}/launch.log
    set_clipboard "${logfile}"
    echo ${logfile}
} # }}}
function latestros2nodelog() { # {{{
    if [ $# -lt 1 ]; then
        echo "[error] wrong number of arguments."
...
function latestros2testlog() { # {{{
    BASE_DIR_PATH=~/workspace/build
    FILE_NAME=Test.xml
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : latestros2testlog <package_name>"
        return 1
    fi
    package_name=$1
    root_dir_path=${BASE_DIR_PATH}/${package_name}
    dir_path=$(latestdirpath ${root_dir_path}/Testing)
    org_file_path=${dir_path}/${FILE_NAME}
    copy_file_path=${root_dir_path}/test_results/${package_name}/${FILE_NAME}
    \cp -f ${org_file_path} ${copy_file_path}
    set_clipboard ${org_file_path}
    echo "original : ${org_file_path}"
    echo "copied   : ${copy_file_path}"
} # }}}
function latestros2testtmplog() { # {{{
    BASE_DIR_PATH=~/workspace/build
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : latestros2testtmplog <package_name>"
        return 1
    fi
    package_name=$1
    root_dir_path=${BASE_DIR_PATH}/${package_name}/Testing/Temporary
    org_file_path=$(latestfilepath ${root_dir_path} "LastTest_*")
    copy_file_path=${root_dir_path}/LastTest.log
    \cp -f ${org_file_path} ${copy_file_path}
    set_clipboard ${org_file_path}
    echo "original : ${org_file_path}"
    echo "copied   : ${copy_file_path}"
} # }}}
function _getros2pkgname() { # {{{
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : _getros2pkgname <pkg_xml_file_path>"
        return 1
    fi
    pkg_xml_file_path=$1
    grep "<name>.*</name>" ${pkg_xml_file_path} | sed "s/.*<name>//g" | sed "s/<\/name>//g"
} # }}}
function cpk() { # {{{
    pkg_xml_file_name=package.xml
    pkg_root_dir=$(searchpath2top ${PWD} ${pkg_xml_file_name})
    #echo ${pkg_root_dir}
    if [ -z ${pkg_root_dir} ]; then
        echo "[error] ${pkg_xml_file_name} does not exist in the upper level directory."
    else
        pkg_xml_file_path=${pkg_root_dir}/${pkg_xml_file_name}
        _getros2pkgname ${pkg_xml_file_path}
    fi
} # }}}
function cpks() { # {{{
    pkg_xml_file_name=package.xml
    pkg_xml_paths_file=$(find . -type f -name ${pkg_xml_file_name} 2> /dev/null)
    pkg_xml_paths_link=$(find . -type l -name ${pkg_xml_file_name} 2> /dev/null)
    pkg_xml_file_paths="${pkg_xml_paths_file} ${pkg_xml_paths_link}"
    # echo ${pkg_xml_paths_file}
    # echo ${pkg_xml_paths_link}
    # echo ${pkg_xml_file_paths}
    for pkg_xml_file_path in $pkg_xml_file_paths
    do
        # echo ${pkg_xml_file_path}
        _getros2pkgname ${pkg_xml_file_path}
    done
} # }}}
function e2q() { # {{{
...
function setresourcepath() { # {{{
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : setresourcepath <pkg_name>"
        return 1
    fi
    pkg_name=$1
    export IGN_GAZEBO_RESOURCE_PATH="$(\cd ~/workspace; source ./install/setup.bash &> /dev/null; ros2 pkg prefix ${pkg_name})/share"
    echo IGN_GAZEBO_RESOURCE_PATH=${IGN_GAZEBO_RESOURCE_PATH}
} # }}}
function addresourcepath() { # {{{
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : addresourcepath <pkg_name>"
        return 1
    fi
    pkg_name=$1
    export IGN_GAZEBO_RESOURCE_PATH="${IGN_GAZEBO_RESOURCE_PATH}:$(\cd ~/workspace; source ./install/setup.bash &> /dev/null; ros2 pkg prefix ${pkg_name})/share"
    echo IGN_GAZEBO_RESOURCE_PATH=${IGN_GAZEBO_RESOURCE_PATH}
} # }}}
function latestignlog() { # {{{
        ROOTDIR=${HOME}/.ignition/gazebo/log
    RAWFILENAME=server_console.log
    COPYFILENAME=latest_server_console.log
    dir_path=$(latestdirpath ${ROOTDIR})
    org_file_path=${dir_path}/${RAWFILENAME}
    copy_file_path=${ROOTDIR}/${COPYFILENAME}
    ln -f ${org_file_path} ${copy_file_path}
    set_clipboard "${copy_file_path}"
    # echo ${org_file_path}
    echo ${copy_file_path}
    # vim ${copy_file_path}
} # }}}
function igngui_dispmodel() { # {{{
    if [ $# -ne 1 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : igngui_dispmodel <model_name>"
        return 1
    fi
    model_name=$1
    # echo "${model_name}"
    ign service -s /gui/view/collisions --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
    ign service -s /gui/view/transparent --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
    ign service -s /gui/view/com --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
    ign service -s /gui/view/inertia --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
#   ign service -s /gui/view/frames --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
#   ign service -s /gui/view/joints --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
#   ign service -s /gui/view/wireframes --reqtype ignition.msgs.StringMsg --reptype ignition.msgs.Boolean --timeout 1000 --req "data: \"${model_name}\""
} # }}}
function exportsdf() { # {{{
    if [ $# -ne 3 ]; then
        echo "[error] wrong number of arguments."
        echo "  usage : exportsdf <world_name> <expand_include_tags> <sdf_file_path>"
        return 1
    fi
    world_name=$1
    expand_include_tags=$2
    sdf_file_path=$3
    ign service -s /world/${world_name}/generate_world_sdf \
        --reqtype ignition.msgs.SdfGeneratorConfig \
        --reptype ignition.msgs.StringMsg \
        --timeout 5000 \
        --req "global_entity_gen_config: {expand_include_tags: {data: ${expand_include_tags}}, save_fuel_version: {data: false}, resources_use_absolute_paths: {data: true}, copy_model_resources: {data: true}}" > ${sdf_file_path}
    sed -i 's/\\n/\n/g' ${sdf_file_path}
    sed -i "s/\\\\'/'/g" ${sdf_file_path}
    sed -i 's/^data: "//g' ${sdf_file_path}
    sed -i 's/^"$//g' ${sdf_file_path}
    sed -i 's/^\n//g' ${sdf_file_path}
...
function ignvisurdf() {
    if [ $# -ne 2 ]; then
        ...
        echo "       e.g.) ignvisurdf ${HOME}/workspace/src/sample_model sample_urdf/work/attack_zero/attack_zero.urdf"
        return 1
    fi
    MODEL_DIR_PATH=$1
    URDF_PATH=$2
    URDF_FILE_NAME=$(basename ${URDF_PATH})
    MODEL_NAME=${URDF_FILE_NAME%%.*}_0001
    URDF_FILE_PATH=${MODEL_DIR_PATH}/${URDF_PATH}
#   echo ${MODEL_NAME}
#   echo ${URDF_FILE_PATH}
