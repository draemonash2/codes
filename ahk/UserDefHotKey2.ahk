...
;* ***************************************************************
	...
	InitWinTempHide() ; {{{
	{
		...
		global gbWinTempHideIsHidden := false
	} ; }}}
	ToggleWinTempHide() { ; {{{
		global gaWinTempHideHiddenHwnds
		...
		if (!gbWinTempHideIsHidden) {
			...
			gaWinTempHideHiddenHwnds := []
			for proc in gasWINTEMPHIDE_TARGETS {
				for hwnd in WinGetList("ahk_exe " proc) {
					gaWinTempHideHiddenHwnds.Push(hwnd)
					WinHide("ahk_id " hwnd)
				}
			}
